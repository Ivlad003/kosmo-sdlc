...
created: 2026-05-13
updated: 2026-05-13
branch: null                         # set by /kosmo-sdlc:implement
spec:
...
figma: null
pr: null                             # set by /kosmo-sdlc:pr
demo:
  scenario: null                     # set by /kosmo-sdlc:validate
  recording: null
  report: null
  applicable: true                   # false → /kosmo-sdlc:validate skips, writes 'na' report
acs:
  - id: AC1
...
- **`spec.hash`** — sha256 of the captured slice at intake. `/kosmo-sdlc:revalidate` recomputes and compares; mismatch = drift.
...
- **`demo.applicable`** — set to `false` when the feature has no UI surface. `/kosmo-sdlc:validate` writes a `na` report instead of running Playwright.
