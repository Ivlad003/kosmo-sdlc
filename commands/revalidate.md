...
# /kosmo-sdlc:revalidate

Phase 6 of the cycle. The "final check before merge". Same engine as `/kosmo-sdlc:validate`, plus drift detection.
...
1. `_/sdlc-config.md` exists. Re-read both frontmatter and Notes body; the Notes body may have changed since /kosmo-sdlc:validate ran.
...
- If different from `frontmatter.spec.hash`:
  ...
  - Ask the user: re-intake the ticket (`/kosmo-sdlc:intake <TICKET> --refresh`), proceed despite drift, or abort.
  - Append journal row: `Spec drift detected: <old-hash> → <new-hash>. User chose <action>.`

### 2. Re-run validation

Delegate to `/kosmo-sdlc:validate <TICKET>`:
...
- spec drift detected → `/kosmo-sdlc:intake TICKET-1 --refresh`
- new reviewer comments → `/kosmo-sdlc:pr-comments <PR>`
- CI still running → wait, then re-run `/kosmo-sdlc:revalidate TICKET-1`
- review not yet approved → ping reviewers; re-run `/kosmo-sdlc:revalidate` once approved
